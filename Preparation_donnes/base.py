...
def analyze_stability_by_month(df, variable, cible, date_var):
    ...
    merged = pd.merge(monthly_volumes, monthly_rates[['col', 'month', 'event_rate', 'non_event_rate']], on=['col', 'month'], how='left')

    # 7. Affichage des informations générales
    ...
    for modality in df[variable].unique():
        ...
        plt.plot(modality_data['month'].astype(str), modality_data['non_event_rate'], label=f'Taux de non-événement (0) - Modalité {modality}', linestyle='--')
...
data = {
    ...
    'date': ['2023-01-01', '2023-01-01', '2023-02-01', '2023-02-01', '2023-03-01', '2023-03-01', '2023-04-01', '2023-04-01', '2023-05-01', '2023-05-01']
}

df = pd.DataFrame(data)

# Appliquer la fonction pour analyser la stabilité des modalités
merged_data = analyze_stability_by_month(df, 'col', 'cible', 'date')
